 ...
 public:
  ...
  content::WebContents* OpenRewardsPopup() {
    ...
    BraveActionsContainer* brave_actions_container =
        brave_location_bar_view->brave_actions_container();
    EXPECT_TRUE(brave_actions_container->ExecuteAction(
        brave_rewards_extension_id, true));
  ...
  void PopulateAutoContributeTable() {
    // Navigate to a verified site in a new tab
    const std::string hostname = "duckduckgo.com";
    GURL url = embedded_test_server()->GetURL(hostname, "/index.html");
  ...
  void TipPublisher(const std::string& publisher, bool verified) {
    ...
    // Construct an observer to wait for the overlay to load
    ...
    const auto& source =
        static_cast<const content::Source<content::WebContents>&>(
            overlay_observer.source());

    content::WebContents* overlay_contents = source.ptr();
    ...
    // TODO(emerick): Make sure that thank you banner shows correct publisher
    // data (domain and amount)

    if (verified) {
      // Wait for reconciliation to complete successfully
      WaitForReconcileCompleted();
      // TODO(emerick): Make sure that balance is updated correctly
    }

    // TODO(emerick): Check that tip table shows the appropriate tip
    // amount
  }
...
IN_PROC_BROWSER_TEST_F(BraveRewardsBrowserTest,
  ...
  // Populate and verify auto contribute table
  PopulateAutoContributeTable();

  // Stop observing the Rewards service
  rewards_service_->RemoveObserver(this);
}

// #4a - Visit verified site
IN_PROC_BROWSER_TEST_F(BraveRewardsBrowserTest, VisitVerifiedSite) {
  // Observe the Rewards service
  rewards_service_->AddObserver(this);
  ...
  // Make sure site appears in auto-contribute table
  {
    content::EvalJsResult js_result = EvalJs(
        ...
        content::EXECUTE_SCRIPT_DEFAULT_OPTIONS,
        content::ISOLATED_WORLD_ID_CONTENT_END);
    EXPECT_STREQ(js_result.ExtractString().c_str(), hostname.c_str());
  }
...
// #4b - Visit unverified site
IN_PROC_BROWSER_TEST_F(BraveRewardsBrowserTest, VisitUnverifiedSite) {
  // Observe the Rewards service
  rewards_service_->AddObserver(this);
  ...
  // Navigate to an unverified site in a new tab
  const std::string hostname = "google.com";
  GURL url = embedded_test_server()->GetURL(hostname, "/index.html");
  ui_test_utils::NavigateToURLWithDisposition(
      browser(), url, WindowOpenDisposition::NEW_FOREGROUND_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_NAVIGATION);
...
IN_PROC_BROWSER_TEST_F(BraveRewardsBrowserTest, AutoContribution) {
  ...
  // Set reconcile time to trigger immediately
  rewards_service()->SetReconcileTime(1);

  // Claim grant using panel
  bool use_panel = true;
  ClaimGrant(use_panel);

  // Populate and verify auto contribute table, which should trigger
  // an immediate contribution due to the short reconcile time set
  // above
  PopulateAutoContributeTable();
  ...
  // TODO(emerick): Make sure that all contribution phases finish correctly and
  // balance is updated correctly

  // TODO(emerick): Check rewards summary and make sure that it reflects it
  // there as well
